...
class NotesColorSubState extends MusicBeatSubstate {
	...
	var dir = (path:String, ?gpu:Bool) -> Paths.image('menus/noteColor/$path', gpu);
	public function new() {
		super();
		...
		var bg = new FlxSprite(720).makeGraphic(FlxG.width - 720, FlxG.height, FlxColor.BLACK);
		bg.alpha = 0.25;
		add(bg);
		var bg = new FlxSprite(750, 160).makeGraphic(FlxG.width - 780, 540, FlxColor.BLACK);
		bg.alpha = 0.25;
		add(bg);
		
		var text = new Alphabet(50, 86, "CTRL", false);
		text.alignment = CENTERED;
		text.setScale(0.4);
		add(text);

		copyButton = new FlxSprite(760, 50).loadGraphic(dir("copy"));
		copyButton.alpha = 0.6;
		add(copyButton);

		pasteButton = new FlxSprite(1180, 50).loadGraphic(dir("paste"));
		...
		colorPalette = new FlxSprite(820, 580).loadGraphic(dir("palette", false));
		colorPalette.scale.set(20, 20);
		colorPalette.updateHitbox();
		colorPalette.antialiasing = false;
		add(colorPalette);
		
		colorWheel = new FlxSprite(860, 200).loadGraphic(dir("colorWheel"));
		...
		var tipX = 20;
		var tipY = 660;
		var tip = new FlxText(tipX, tipY, 0, Language.getPhrase("note_colors_tip", "Press RESET to Reset the selected Note Part."), 16);
	...
	override function update(elapsed:Float) {
		...
		if (controls.controllerMode != _lastControllerMode) {
			...
			} /*else { // changed to keyboard mid state
		...
		if (holdingOnObj != null) { // holding
			if (FlxG.mouse.justReleased || (controls.controllerMode && controls.justReleased("accept"))) {
				...
				if (holdingOnObj == colorGradient) {
					...
					var center = new FlxPoint(colorWheel.x + colorWheel.width / 2, colorWheel.y + colorWheel.height / 2);
			...
			if (FlxG.keys.pressed.SHIFT || FlxG.gamepads.anyPressed(LEFT_SHOULDER)) {
				for (i in 0...3) {
					...
					var color:FlxColor = !onPixel ?
					ClientPrefs.defaultData.arrowRGB[curSelectedNote][i] :
	...
	function centerHexTypeLine() { //trace(hexTypeNum);
		if (hexTypeNum > 0) {
	...
	function makeColorAlphabet(x:Float = 0, y:Float = 0):Alphabet { // alphabets
		var text = new Alphabet(x, y, "", true);
		text.alignment = CENTERED;
		text.setScale(0.6);
		add(text);
		return text;
	...
	public function spawnNotes() {
		...
		skinNote = new FlxSprite(48, 24).loadGraphic(dir("note" + (!onPixel ? "" : "Pixel")), true, res, res);
		...
		for (i in 0...3) {
			var newNote = new FlxSprite(230 + (100 * i), 100).loadGraphic(dir("note" + (!onPixel ? "" : "Pixel")), true, res, res);
		...
		for (i in 0...dataArray.length) {
			...
			var newNote = new StrumNote(150 + (480 / dataArray.length * i), 200, i, 0);
