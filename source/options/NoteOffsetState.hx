...
class NoteOffsetState extends MusicBeatState {
	...
	override public function create() {
		#if DISCORD_ALLOWED DiscordClient.changePresence("Delay/Combo Offset Menu", null); #end
		...
		states.editors.CharacterEditorState.defaultStage(this); // Stage
		...
		rating = new FlxSprite().loadGraphic(Paths.image("ui/judgements/sick"));
		...
		for (i in seperatedScore) {
			var numScore = new FlxSprite(43 * daLoop).loadGraphic(Paths.image('ui/combo/num' + i));
		...
		beatText = new Alphabet(0, 0, Language.getPhrase("delay_beat_hit", "Beat Hit!"), true); // Note delay stuff
		...
		var blackBox = new FlxSprite().makeGraphic(FlxG.width, 40, FlxColor.BLACK);
	...
	override public function update(elapsed:Float) {
		...
		if (onComboMenu) {
			...
			if (controls.controllerMode) { // controller things
	...
	function createTexts() {
		for (i in 0...4) {
			var text = new FlxText(10, 48 + (i * 30), 0, "", 24);
	...
	function updateMode() {
		...
		else str = Language.getPhrase("note_delay", "Note/Beat Delay");

		if (!controls.controllerMode) str2 = Language.getPhrase("switch_on_accept", "(Press Accept to Switch)");
		else str2 = Language.getPhrase("switch_on_start", "(Press Start to Switch)");
