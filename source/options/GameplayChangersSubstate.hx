...
class GameplayChangersSubstate extends MusicBeatSubstate {
	...
	function getOptions() {
		var goption = new GameplayOption("Scroll Type",
			"scrolltype",
			STRING, 
			"multiplicative",
		...
		#if FLX_PITCH
		var option = new GameplayOption("Playback Rate", "songspeed", FLOAT, 1);
	...
	public function new() {
		...
		for (i in 0...optionsArray.length) {
			var optionText = new Alphabet(150, 360, optionsArray[i].name, true);
			...
			if (optionsArray[i].type == BOOL) {
				...
				var checkbox = new CheckboxThingie(optionText.x - 105, optionText.y, optionsArray[i].getValue() == true);
				...
				optionText.snapToPosition();
				var valueText = new AttachedText(Std.string(optionsArray[i].getValue()), optionText.width + 40, 0, true, 0.8);
