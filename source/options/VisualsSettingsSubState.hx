...
class VisualsSettingsSubState extends BaseOptionsMenu {
	...
	public function new() {
		...
		for (i in 0...Note.colArray.length) { // for note skins and splash skins
			var note = new StrumNote(370 + (560 / Note.colArray.length) * i, -200, i, 0);
			changeNoteSkin(note);
			notes.add(note);
		...
		// options
		var path:String = "images/notes"; // Path List Shit
		var noteSkins:Array<String> = Mods.mergeAllTextsNamed('$path/skins/list.txt');
		if (noteSkins.length > 0) {
			if (!noteSkins.contains(ClientPrefs.data.noteSkin))
				ClientPrefs.data.noteSkin = ClientPrefs.defaultData.noteSkin; //Reset to default if saved noteskin couldnt be found

			noteSkins.insert(0, ClientPrefs.defaultData.noteSkin); //Default skin always comes first
			var option = new Option("Note Skins:",
		...
		var noteSplashes:Array<String> = Mods.mergeAllTextsNamed('$path/splashes/list.txt');
		if (noteSplashes.length > 0) {
			if (!noteSplashes.contains(ClientPrefs.data.splashSkin))
				ClientPrefs.data.splashSkin = ClientPrefs.defaultData.splashSkin; //Reset to default if saved splashskin couldnt be found

			noteSplashes.insert(0, ClientPrefs.defaultData.splashSkin); //Default skin always comes first
			var option = new Option("Note Splashes:",
		...
		var option = new Option("Note Splash Opacity",
		...
		var option = new Option("Hide HUD",
			"If checked, hides most HUD elements.",
			"hideHud",
			BOOL);
		addOption(option);

		var option = new Option("Time Bar:",
		...
		var option = new Option("Flashing Lights",
			"Uncheck this if you're sensitive to flashing lights!",
			"flashing",
			BOOL);
		addOption(option);

		var option = new Option("Camera Zooms",
			"If unchecked, the camera won't zoom in on a beat hit.",
			"camZooms",
			BOOL);
		addOption(option);

		var option = new Option("Score Text Grow on Hit'",
			"If unchecked, disables the Score text growing\neverytime you hit a note.",
			"scoreZoom",
			BOOL);
		addOption(option);

		var option = new Option("Health Bar Opacity",
		...
		#if !mobile
		var option = new Option("FPS & RAM Counter",
			"If it's '0', it hides the FPS and RAM; '1' shows only the FPS; and '2' shows both the FPS and RAM.",
			"showFPS", INT);
		option.minValue = 0;
		option.maxValue = 2;
		addOption(option);
		...
		var option = new Option("Pause Music:",
		...
		var option = new Option("Check for Updates",
		...
		#if DISCORD_ALLOWED
		var option = new Option("Discord Rich Presence",
		...
		var option = new Option("Combo Stacking",
