...
class SoundTray extends flixel.system.ui.FlxSoundTray {
	...
	override public function update(MS:Float):Void {
		...
		if (_timer > 0) {
			...
			visible = false;
			active = false;
			ClientPrefs.sysData.volume = FlxG.sound.volume;
			ClientPrefs.sysData.muted = FlxG.sound.muted;
			ClientPrefs.saveSys();
		}
	...
	inline function getColor(v:Float):FlxColor {
		...
		if (v <= 0.2) return FlxColor.interpolate(0xFF0B5F1E, FlxColor.GREEN, v / 0.2);
		if (v <= 0.4) return FlxColor.interpolate(FlxColor.GREEN, FlxColor.YELLOW, (v - 0.2) / 0.2);
		if (v <= 0.6) return FlxColor.YELLOW;
		if (v <= 0.8) return FlxColor.interpolate(FlxColor.YELLOW, FlxColor.ORANGE, (v - 0.6) / 0.2);
