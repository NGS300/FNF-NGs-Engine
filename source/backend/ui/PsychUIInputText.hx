...
class PsychUIInputText extends FlxSpriteGroup {
	...
	public var selectedFormat = new FlxTextFormat(FlxColor.WHITE);
	...
	function onKeyDown(e:KeyboardEvent) {
		...
		switch (keyCode) { // Fix missing cedilla
			case 231: charCode = e.shiftKey ? 0xC7 : 0xE7; //ç and Ç
		}

		if (e.controlKey) { // Control key actions
			switch (flxKey) {
				...
				case DELETE:
					if (selectIndex < 0 || selectIndex == caretIndex) { // This is| a test <-> This is test
						...
						var lastText = text;
						if (space > -1) text = text.substr(0, caretIndex) + text.substring(caretIndex + space + spc);
		...
		switch (flxKey) {
			...
			case END: //move caret to the end
				...
				caretIndex = text.length;
			case INSERT: inInsertMode = !inInsertMode; //change to insert mode
			...
			case E, I, U: //these support grave, acute and circumflex
				...
				if (_nextAccent == GRAVE || _nextAccent == ACUTE || _nextAccent == CIRCUMFLEX) charCode += grave - capital + _nextAccent; //Supported accents
				else if (_nextAccent == TILDE) _typeLetter(getAccentCharCode(_nextAccent)); //Unsupported accent
			...
			case N: //it only supports tilde
				...
				_typeLetter(charCode);
				_nextAccent = NONE;
			case ESCAPE: focusOn = null;
			case ENTER: onPressEnter(e);
			default:
				if (charCode < 1)
	...
	public dynamic function onPressEnter(e:KeyboardEvent) focusOn = null;
	...
	public static function getAccentCharCode(accent:AccentCode) {
		switch (accent) {
			...
			case CIRCUMFLEX: return 0x5E;
			case ACUTE: return 0xB4;
			case GRAVE: return 0x60;
			default: return 0x0;
		}
	}
	...
	function set_forceCase(v:CaseMode) { // from FlxInputText
		forceCase = v;
		text = filter(text);
		return forceCase;
	}
	...
	function filter(text:String):String {
		...
		if (filterMode != NO_FILTER) {
			...
			switch (filterMode) {
				...
				default: throw new flash.errors.Error("FlxInputText: Unknown filterMode (" + filterMode + ")");
			}
