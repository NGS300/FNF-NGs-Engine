...
class PsychUISlider extends FlxSpriteGroup {
	...
	function _updateHandleX() handle.x = bar.x - handle.width / 2 + FlxMath.remapToRange(FlxMath.roundDecimal(value, decimals), min, max, 0, bar.width);
	...
	function get_label() return labelText.text;
}