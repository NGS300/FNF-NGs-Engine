...
class LuaUtils {
	...
	public static function setVarInArray(instance:Dynamic, variable:String, value:Dynamic, allowMaps:Bool = false):Any {
		...
		if (allowMaps && isMap(instance)) { //trace(instance);
	...
	public static function getModSetting(saveTag:String, ?modName:String = null) {
		...
		if (FileSystem.exists(path)) {
			if (settings == null || !settings.exists(saveTag)) {
				...
				try { //FunkinLua.luaTrace('getModSetting: Trying to find default value for "$saveTag" in Mod: "$modName"');
					...
					for (i in 0...parsedJson.length) {
						...
						if (sub != null && sub.save != null && !settings.exists(sub.save)) {
							if (sub.type != "keybind" && sub.type != "key")
								if (sub.value != null) //FunkinLua.luaTrace('getModSetting: Found unsaved value "${sub.save}" in Mod: "$modName"');
									settings.set(sub.save, sub.value);
							else //FunkinLua.luaTrace('getModSetting: Found unsaved keybind "${sub.save}" in Mod: "$modName"');
	...
	public static function isMap(variable:Dynamic) {
		switch (Type.typeof(variable)) {
			case ValueType.TClass(haxe.ds.StringMap) | ValueType.TClass(haxe.ds.ObjectMap) | ValueType.TClass(haxe.ds.IntMap) | ValueType.TClass(haxe.ds.EnumValueMap): return true;
	...
	public static function loadFrames(spr:FlxSprite, image:String, spriteType:String) {
		switch (spriteType.toLowerCase().replace(" ", "")) {
			...
			case "aseprite", "ase", "json", "jsoni8": spr.frames = Paths.getAsepriteAtlas(image);
			case "packer", "packeratlas", "pac": spr.frames = Paths.getPackerAtlas(image);
			case "sparrow", "sparrowatlas", "sparrowv2": spr.frames = Paths.getSparrowAtlas(image);
			default: spr.frames = Paths.getAtlas(image);
		}
	}
	...
	public static function cancelTween(tag:String) {
		var i = "tween_";
		if (!tag.startsWith(i)) tag = i + LuaUtils.formatVariable(tag);
		var variables = MusicBeatState.getVariables();
	...
	public static function getTweenTypeByString(?type:String = "") { //buncho string stuffs
