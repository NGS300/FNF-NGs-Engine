...
class DialogueBoxPsych extends FlxSpriteGroup {
	...
	public var dialogueSound = ["dialogue/dialogue", "dialogue/dialogueClose"];

	public function new(dialogueList:DialogueFile, ?song:String = null) {
		super();
		Paths.sound(dialogueSound[0]);
		Paths.sound(dialogueSound[1]);
		...
		box.frames = Paths.getSparrowAtlas("ui/speech_bubble");
		box.scrollFactor.set();

		var anims:Array<Dynamic> = [
			["normal", "peech bubble normal", true],
			["normalOpen", "Speech Bubble Normal Open", false],
			["angry", "AHH speech bubble", true],
			["angryOpen", "speech bubble loud open", false],
			["center-normal", "speech bubble middle", true],
			["center-normalOpen", "Speech Bubble Middle Open", false],
			["center-angry", "AHH Speech Bubble middle", true],
			["center-angryOpen", "speech bubble Middle loud open", false]
		];

		for (a in anims)
			box.animation.addByPrefix(a[0], a[1], 24, a[2]);
	...
	function spawnCharacters() {
		...
		for (individualChar in charsMap.keys()) {
			...
			var char = new DialogueCharacter(x + offsetPos, y, individualChar);
		...
		if (!dialogueEnded) {
			...
			if (Controls.instance.ACCEPT || back) {
				if (!daText.finishedText && !back) {
					...
					startNextDialog();
				FlxG.sound.play(Paths.sound(dialogueSound[1]), closeVolume);
			} else if (daText.finishedText) {
				var char = arrayCharacters[lastCharacter];
	...
	function startNextDialog():Void {
		...
		if (daText.sound == null || daText.sound.trim() == "")
			daText.sound = dialogueSound[1];

		daText.y = DEFAULT_TEXT_Y;
		if (daText.rows > 2)
			daText.y -= LONG_TEXT_ADD;

		var char = arrayCharacters[character];
		if (char != null) {
	...
	inline public static function parseDialogue(path:String):DialogueFile
		return cast (!CoolUtil.exists(path)) ? dummy() : CoolUtil.getJson(path);
