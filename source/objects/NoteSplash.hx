...
class NoteSplash extends FlxSprite {
	...
	public static var defaultNoteSplash(default, never):String = "notes/splashes/noteSplashes";
	...
	public function loadSplash(?splash:String) {
		...
		if (configs.exists(path)) {
			...
			var config:Dynamic = CoolUtil.getJson(Paths.getPath('$path.json', TEXT)); // haxe.Json.parse(Paths.getTextFromFile('$path.json'));
			if (config != null) {
	...
	public function spawnSplashNote(?x:Float = 0, ?y:Float = 0, ?noteData:Int = 0, ?note:Note, ?randomize:Bool = true) {
		...
		if (config.allowRGB) {
			...
			if (inEditor || (note == null || note.noteSplashData.useRGBShader) && (PlayState.SONG == null || !PlayState.SONG.disableNoteRGB)) {
				...
				if ((note == null || !note.noteSplashData.useGlobalShader) || inEditor) { // If Note RGB is enabled:
