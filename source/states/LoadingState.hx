...
class LoadingState extends MusicBeatState {
	...
	override function create() {
		...
		var barBack = new FlxSprite(0, 660).makeGraphic(1, 1, FlxColor.BLACK);
	...
	static function _startPool() {
		...
		#end*/
		var threadCount:Int = (#if !MULTITHREADED_LOADING 1 #else Std.int(Math.max(1, getCPUThreadsCount() - #if DISCORD_ALLOWED 2 #else 1 #end)) #end);
	...
	public static function prepareToSong() {
		...
		new Future<Bool>(() -> {
			var noteSkin:String = Note.defaultNoteSkin; // LOAD NOTE IMAGE
			...
			var path = Note.defaultNotePath;
			if (Paths.fileExists('images/$path' + '$customSkin.png', IMAGE)) noteSkin = customSkin;
			imagesToPrepare.push(path + noteSkin);

			var noteSplash:String = NoteSplash.defaultNoteSplash; // LOAD NOTE SPLASH IMAGE
			...
			try {
				...
				var path:String = '$folder/preload';
				var moddyFile:String = Paths.modsJson(path);
				var json:Dynamic = CoolUtil.getJson(CoolUtil.exists(moddyFile) ? moddyFile : Paths.json(path));
				...
				#end*/
				//var moddyFile:String = Paths.modsJson('$folder/preload');
				//json = CoolUtil.getJson(CoolUtil.exists(moddyFile) ? moddyFile : path);
	...
	static function _threadFunc() {
		...
		for (image in imagesToPrepare) initThread(() -> preloadGraphic(image), 'image $image'); // for images, they get to have their own thread
	}

	static function initThread(func:Void->Dynamic, traceData:String) {
		// trace('scheduled $func in threadPool');
		#if debug var threadSchedule = Sys.time(); #end
		threadPool.run(() -> {
			#if debug
	...
	inline private static function preloadCharacter(char:String, ?prefixVocals:String) {
		try {
			...
			var character:Dynamic = CoolUtil.getJson(path);
			/*#if MODS_ALLOWED
	...
	static function preloadSound(key:String, ?path:String, ?modsAllowed:Bool = true, ?beepOnNull:Bool = true):Null<Sound> { // thread safe sound loader
	...
	static function preloadGraphic(key:String):Null<BitmapData> { // thread safe sound loader
		try {
