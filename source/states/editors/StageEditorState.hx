...
class StageEditorState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent {
	...
	var selectionSprites = new FlxSpriteGroup();
	override function create() {
		...
		for (i in 0...4) {
			var spr = new FlxSprite().makeGraphic(1, 1, FlxColor.LIME);
			spr.alpha = 0.8;
	...
	function addHelpScreen() {
		...
		for (i => txt in str) {
			...
			var helpText = new FlxText(0, 0, 680, txt, 16);
	...
	function screenUI() {
		...
		var bg = new FlxSprite(0, FlxG.height - 60).makeGraphic(1, 1, FlxColor.BLACK);
		bg.cameras = [camHUD];
		bg.alpha = 0.4;
		...
		var tipText = new FlxText(0, FlxG.height - 44, 300, "Press F1 for Help", 20);
		...
		var targetTxt = new FlxText(30, FlxG.height - 52, 300, "Camera Target", 16);
	...
	function addSpriteListBox() {
		...
		var buttonMoveUp = new PsychUIButton(buttonX, buttonY, "Move Up", function() {
		...
		var buttonMoveDown = new PsychUIButton(buttonX, buttonY + 30, "Move Down", function() {
			var selected:Int = spriteListRadioGroup.checked;
			if (selected < 0) return;
		...
		var buttonCreate = new PsychUIButton(buttonX, buttonY + 60, "New", function() createPopup.visible = createPopup.active = true);
		...
		var buttonDuplicate = new PsychUIButton(buttonX, buttonY + 90, "Duplicate", function() {
			var selected:Int = spriteListRadioGroup.checked;
			if (selected < 0) return;
			...
			var copiedMeta = new StageEditorMetaSprite(null, copiedSpr);
		...
		var buttonDelete = new PsychUIButton(buttonX, buttonY + 120, "Delete", function() {
			var selected:Int = spriteListRadioGroup.checked;
			if (selected < 0) return;
	...
	function spriteCreatePopup() {
		...
		var txt = new FlxText(0, bg.y + 10, 180, "New Sprite", 24);
		txt.screenCenter(X);
		txt.alignment = CENTER;
		createPopup.add(txt);

		var btnY = 320;
		var btn = new PsychUIButton(0, btnY, "No Animation", function() loadImage("sprite"));
		btn.screenCenter(X);
		createPopup.add(btn);

		btnY += 50;
		var btn = new PsychUIButton(0, btnY, "Animated", function() loadImage("animatedSprite"));
		btn.screenCenter(X);
		createPopup.add(btn);

		btnY += 50;
		var btn = new PsychUIButton(0, btnY, "Solid Color", function() {
	...
	function addDataTab() {
		...
		var saveButton = new PsychUIButton(UI_box.width - 90, UI_box.height - 50, "Save", function() saveData());
	...
	function addObjectTab() {
		...
		nameInputText.onChange = function(old:String, cur:String) { // change name
		...
		var imgButton = new PsychUIButton(objX, objY, "Change Image", function() {
		...
		var animationsButton = new PsychUIButton(objX + 90, objY, "Animations", function() {
			var selected = getSelected();
			if (selected == null)
				return;
		...
		colorInputText.onChange = function(old:String, cur:String) { // change color
			var selected = getSelected();
			if (selected != null)
		...
		function updateScroll() { // scroll factor
			var selected = getSelected();
			if (selected != null)
		...
		alphaStepper.onValueChange = function() { // alpha/opacity
			var selected = getSelected();
			if (selected != null)
		...
		antialiasingCheckbox.onClick = function() { // antialiasing
			var selected = getSelected();
			if (selected != null) {
		...
		angleStepper.onValueChange = function() { // alpha/opacity
			var selected = getSelected();
			if (selected != null)
		...
		function updateFlip() { //flip X and flip Y
			var selected = getSelected();
			if (selected != null) {
				if (selected.type != "square") {
		...
		objY += 45;
		function recalcFilter() { // low and/or high quality
			var selected = getSelected();
			if (selected != null) {
				var filt = 0;
	...
	function addMetaTab() {
		...
		var objX = 10;
		var objY = 20;
		var openPreloadButton = new PsychUIButton(objX, objY, "Preload List", function() {
	...
	function addStageTab() {
		...
		var reloadStage = new PsychUIButton(140, 10, "Reload", function() {
			#if DISCORD_ALLOWED DiscordClient.changePresence("Stage Editor", 'Stage: $lastLoadedStage'); #end
		...
		var dummyStage = new PsychUIButton(140, 40, "Load Template", function() {
	...
	function updateStageDataUI() {
		uiInputText.text = (stageJson.stageUI != null ? stageJson.stageUI : ""); //input texts

		//checkboxes
		hideGirlfriendCheckbox.checked = (stageJson.hide_girlfriend);
		gf.visible = !hideGirlfriendCheckbox.checked;

		zoomStepper.value = FlxG.camera.zoom = stageJson.defaultZoom; //steppers
		...
		switch (id) {
			case PsychUIRadioGroup.CLICK_EVENT, PsychUIBox.CLICK_EVENT:
				if (sender == spriteListRadioGroup || sender == UI_box)
					checkUIOnObject();
			case PsychUICheckBox.CLICK_EVENT: unsavedProgress = true;
			case PsychUIInputText.CHANGE_EVENT, PsychUINumericStepper.CHANGE_EVENT: unsavedProgress = true;
		}
	}
	...
	function focusOnTarget(target:String) {
		var focusPoint = FlxPoint.weak(0, 0);
		switch (target) {
			case "boyfriend":
	...
	function onLoadComplete(_):Void {
		...
		if (fullPath != null) {
			...
			openSubState(new BasePrompt(480, 160, 'This file is not inside Engine.', function(state:BasePrompt) {
				var txt = new FlxText(0, state.bg.y + 60, 460, 'Copy to: "$modFolder"?', 11);
				txt.alignment = CENTER;
				txt.screenCenter(X);
				...
				var btnY = 390;
				var btn = new PsychUIButton(0, btnY, "OK", function() {
				...
				var btn = new PsychUIButton(0, btnY, "Cancel", function() {
					_makeNewSprite = null;
					state.close();
				});
				btn.screenCenter(X);
...
class StageEditorMetaSprite {
	...
	public var type:String; // basic variables for all types
...
class StageEditorAnimationSubstate extends MusicBeatSubstate {
	...
	public function new() {
		super();
		var grid = new FlxBackdrop(FlxGridOverlay.createGrid(50, 50, 100, 100, true, 0xFFAAAAAA, 0xFF666666));
		add(grid);
	...
	function addAnimationsUI() {
		...
		var initAnimButton = new PsychUIButton(160, animationDropDown.y, "Main Animation", function() {
		...
		var addUpdateButton = new PsychUIButton(40, animationIndicesInputText.y + 35, "Add/Update", function() {
		...
		var removeButton = new PsychUIButton(160, animationIndicesInputText.y + 35, "Remove", function() {
			for (anim in target.animations) {
				if (animationInputText.text == anim.anim) {
	...
	override function update(elapsed:Float) {
		...
		if (target.animations.length > 1) { // ANIMATION SCROLLING
		...
		if (target.sprite.animation.curAnim != null) { // OFFSET
			var spr:ModchartSprite = cast (target.sprite, ModchartSprite);
