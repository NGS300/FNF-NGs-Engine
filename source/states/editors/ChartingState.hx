...
class ChartingState extends MusicBeatState implements PsychUIEventHandler.PsychUIEvent {
	...
	override function create() {
		...
		var editDir = "ui/editors";
		vortexIndicator = new FlxSprite(gridBg.x - GRID_SIZE, FlxG.height/2).loadGraphic(Paths.image('$editDir/vortex_indicator'));
		...
		if (SHOW_EVENT_COLUMN) {
			eventIcon = new FlxSprite(0, iconY).loadGraphic(Paths.image('$editDir/eventIcon'));
		...
		autoSaveIcon = new FlxSprite(50).loadGraphic(Paths.image('$editDir/autosave'));
	...
	override function update(elapsed:Float) {
		...
		if (FlxG.mouse.x >= minX && FlxG.mouse.x < gridBg.x + gridBg.width) {
			...
			if (isMovingNotes) { // Move note data
				...
				if (dummyArrow.y != movingNotesLastY) { // Move note strum time
		...
		if (Conductor.songPosition != lastTime || forceDataUpdate) {
			...
			if (metronomeStepper.value > 0 && lastBeatHit != curBeat) // moved from beatHit()
	...
	function reloadNotesDropdowns() {
		if (eventDropDown != null) { // Event drop down
			eventsList = [];
			
			var currentStage:String = PlayState.SONG.stage.toLowerCase(); 
			...
			for (file in eventFiles) {
				...
				var isStageSpecific:Bool = false;
				var targetStages:Array<String> = [];
				var cleanDescLines:Array<String> = [];

				var lines:Array<String> = desc.split('\n');
				for (line in lines) {
					var trimmed = StringTools.trim(line);
					if (trimmed.startsWith("Stage:")) {
						isStageSpecific = true;
						var rawStages = trimmed.substring(6).split(',');
						for (s in rawStages)
							targetStages.push(StringTools.trim(s).toLowerCase());
					} else
						cleanDescLines.push(line);
				}

				var cleanDesc:String = StringTools.trim(cleanDescLines.join('\n'));
				if (!isStageSpecific || targetStages.contains(currentStage))
					eventsList.push([file, cleanDesc]);
			}
		...
		if (noteTypeDropDown != null) { // Note type drop down
	...
	function addSongTab() {
		...
		var characters:Array<String> = []; // Find characters
		objY += 40;
	...
	function addFileTab() {
		...
		btnY++;
		btnY += 20;
		var btn = new PsychUIButton(btnX, btnY, '  Save', function() {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		btnY += 20;
		var btn = new PsychUIButton(btnX, btnY, '  Save as...', function() {
			if (!fileDialog.completed) return;
			upperBox.isMinimized = true;
			upperBox.bg.visible = false;
		...
		if (SHOW_EVENT_COLUMN) {
			btnY += 20;
			var btn = new PsychUIButton(btnX, btnY, "  Save Events...", function() {
				if (!fileDialog.completed) return;
				upperBox.isMinimized = true;
	
				updateChartData();
				fileDialog.save("events.json", PsychJsonPrinter.print({events: PlayState.SONG.events, format: "nge_v0"}, ["events"]),
		...
		var btn:PsychUIButton = new PsychUIButton(btnX, btnY, '  Update (Legacy)...', function() {
			...
			fileDialog.open(function() {
				...
				try {
					...
					if (loadedChart == null || !Reflect.hasField(loadedChart, 'song')) { //Check if chart is ACTUALLY a chart and valid
						showOutput('Error: File loaded is not a ${CoolUtil.engine.name} 0.x.x/FNF 0.2.x.x chart.', true);
						return;
					}
	...
	public function UIEvent(id:String, sender:Dynamic) { //trace(id, sender);
		switch(id) {
