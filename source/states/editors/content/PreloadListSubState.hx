...
class PreloadListSubState extends MusicBeatSubstate implements PsychUIEvent {
	...
	override function create() {
		...
		var titleText = new FlxText(0, bg.y + 30, 400, 'Preload List', 24);
		titleText.screenCenter(X);
		titleText.alignment = CENTER;
		titleText.cameras = cameras;
		add(titleText);

		var btn = new PsychUIButton(bg.x + bg.width - 40, bg.y, 'X', close, 40);
		btn.cameras = cameras;
		add(btn);
		...
		var loadFileBtn = new PsychUIButton(0, bg.y + bg.height - 40, 'Load File', function() {
		...
		var loadFolderBtn = new PsychUIButton(0, bg.y + bg.height - 40, 'Load Folder', function() {
			if (!fileDialog.completed) return;
		...
		var saveBtn = new PsychUIButton(0, bg.y + bg.height - 40, 'Save', function() {
			if (!fileDialog.completed) return;
