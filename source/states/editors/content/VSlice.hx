...
class VSlice {
			...
			for (num => event in remainingEvents) {
				...
				if (event.v != null) {
					switch (Type.typeof(event.v)) {
						...
						case TClass(String): fields.push(event.v);
						case TClass(Array):
	...
	static var generatedBy:String = '$generatedName Exporter';
	public static function export(songData:SwagSong, ?difficultyName:String = null):VSlicePackage {
		...
		if (songData.notes != null) {
			for (section in songData.notes) {
				if (section.sectionNotes != null && section.sectionNotes.length > 0) { // Add notes
