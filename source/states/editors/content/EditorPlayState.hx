...
class EditorPlayState extends MusicBeatSubstate { // Borrowed from original PlayState
	...
	override function create() {
		...
		var tipText = new FlxText(10, FlxG.height - 24, 0, "Press ESC to Go Back to Chart Editor", 16);
		...
		#if DISCORD_ALLOWED DiscordClient.changePresence("Playtesting on Chart Editor", PlayState.SONG.song, null, true, songLength); #end // Updating Discord Rich Presence (with Time Left)
		updateScore();
		cachePopUpScore();
	...
	function generateSong() { // Borrowed from PlayState
		...
		for (note in _noteList) { // Load Notes
			...
			if (idx != 0) { // CLEAR ANY POSSIBLE GHOST NOTES
			...
			var swagNote = new Note(note.strumTime, note.noteData, oldNote, false, this);
			...
			if (roundSus > 0) {
				for (susNote in 0...roundSus) {
					...
					var sustainNote = new Note(swagNote.strumTime + (curStepCrochet * susNote), note.noteData, oldNote, true, this);
	...
	public function finishSong():Void {
		if (ClientPrefs.data.noteOffset <= 0)
			...
			finishTimer = new FlxTimer().start(ClientPrefs.data.noteOffset / 1000, (_) -> endSong());
	}
	...
	function cachePopUpScore() {
		var path = (id:String) -> Paths.image('ui/$id' + PlayState.uiPostfix);
		for (judges in ratingsData) path('judgements/${PlayState.uiPrefix}${judges.image}');
		for (i in 0...10) path('combo/${PlayState.uiPrefix}num$i');
	}
	...
	function popUpScore(note:Note = null):Void {
		...
		var judges:Rating = Conductor.judgeNote(ratingsData, noteDiff / playbackRate); //tryna do MS based judgment due to popular demand
		...
		if (PlayState.stageUI != "normal") antialias = !PlayState.isPixelStage;

		rating.loadGraphic(Paths.image('ui/judgements/${PlayState.uiPrefix}' + '${judges.image}${PlayState.uiPostfix}'));
		rating.screenCenter();
		...
		var path = (id:String) -> Paths.image('ui/combo/${id}${PlayState.uiPostfix}');
		var comboSpr = new FlxSprite().loadGraphic(path('${PlayState.uiPrefix}combo'));
		...
		for (i in 0...separatedScore.length) {
			var numScore = new FlxSprite().loadGraphic(path('${PlayState.uiPrefix}num' + Std.parseInt(separatedScore.charAt(i))));
	...
	function keyPressed(key:Int) {
		...
		var plrInputNotes:Array<Note> = notes.members.filter(function(n:Note) // obtain notes that the player can hit
	...
	function opponentNoteHit(note:Note):Void {
		...
		var strum = opponentStrums.members[Std.int(Math.abs(note.noteData))];
		if (strum != null) {
	...
	function spawnNoteSplashOnNote(note:Note) {
		if (note != null) {
			...
			if (strum != null) spawnNoteSplash(strum.x, strum.y, note.noteData, note, strum);
		}
	}
	...
	function updateScore() scoreTxt.text = 'Hits: $songHits | Misses: $songMisses';
}