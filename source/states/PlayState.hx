...
class PlayState extends MusicBeatState {
	...
	static function set_stageUI(value:String):String {
		...
		if (value != "normal") {
			...
			var folderName:String = value.split("-pixel")[0].trim();
	...
	public static var instance:PlayState; // Lua
	#if LUA_ALLOWED public var luaArray:Array<FunkinLua> = []; #end

	#if (LUA_ALLOWED || HSCRIPT_ALLOWED) var luaDebugGroup:FlxTypedGroup<psychlua.DebugLuaText>; #end
	...
	override public function create() {
		...
		#if DISCORD_ALLOWED // String that contains the mode defined here so it isn't necessary to call changePresence for each mode
		...
		else if (stageData.isPixelStage == true) stageUI = "pixel"; //Backward compatibility
		...
		if (curStage == "default") CharacterEditorState.defaultStage(this);
		...
		var foldersToRead:Array<String> = ["scripts/", "scripts/plugins/", "stages/objects/", 'stages/objects/$curStage/'];
		for (subFolder in foldersToRead) {
			for (folder in Mods.directoriesWithFile(Paths.getSharedPath(), subFolder)) {
				for (file in FileSystem.readDirectory(folder)) {
					#if LUA_ALLOWED
					if (file.toLowerCase().endsWith(".lua"))
						if (!file.toLowerCase().endsWith('$curStage.lua'))
							new FunkinLua(folder + file);
					#end
					#if HSCRIPT_ALLOWED
					if (file.toLowerCase().endsWith(".hx"))
						if (!file.toLowerCase().endsWith('$curStage.hx'))
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED) // STAGE SCRIPTS
		...
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED) // SONG SPECIFIC SCRIPTS
	...
	public function startVideo(name:String, forMidSong:Bool = false, canSkip:Bool = true, loop:Bool = false, playOnLoad:Bool = true) {
		...
		if (#if sys FileSystem.exists(fileName) || #end OpenFlAssets.exists(fileName)) { // (#if sys FileSystem.exists(fileName) #else OpenFlAssets.exists(fileName) #end)
			foundFile = true;
			...
			var name:String = 'Video not found: $fileName';
			#if (LUA_ALLOWED || HSCRIPT_ALLOWED) addTextToDebug(name, FlxColor.RED) #else FlxG.log.warn(name) #end;
		}
		#else
	...
	public var psychDialogue:DialogueBoxPsych; //You don't have to add a song, just saying. You can just do "startDialogue(DialogueBoxPsych.parseDialogue(Paths.json('$songName/dialogue')))" and it should load dialogue.json
	...
	public static function getIntroImages():Array<String> {
		var isNormal:Bool = stageUI == "normal";
		var isPixel:Bool = stageUI == "pixel";
		var path:String = "ui/intro";
		return ["ready", "set", isPixel ? "date" : "go"].map(i ->
			isPixel ? '$path/pixel/$i-pixel' : '$path/${uiPrefix}$i' + (isNormal ? "" : uiPostfix)
		);
	}

	function cacheCountdown() {
		var path = (id:String) -> 'intro/${uiPrefix}intro${id}${uiPostfix}';

		var introAssets = new Map<String, Array<String>>();
		introAssets.set(stageUI, getIntroImages());

		var introAlts:Array<String> = introAssets.get(stageUI);
		for (asset in introAlts)
			Paths.image(asset);

		var goSound:String = "Go";
		if ((stageUI == "pixel" || uiPostfix == "-pixel") && CoolUtil.exists(Paths.getPath(path("Date") + '.${Paths.SOUND_EXT}', SOUND)))
			goSound = "Date";

		for (i in ["3", "2", "1", goSound]) Paths.sound(path(i));
	}

	inline function createCountdownSprite(image:String, antialias:Bool):FlxSprite {
		var spr = new FlxSprite().loadGraphic(Paths.image(image));
		spr.cameras = [camHUD];
	...
	public function startCountdown() {
		...
		if (ret != LuaUtils.Function_Stop) {
			...
			for (i in 0...opponentStrums.length) { //if (ClientPrefs.data.middleScroll) opponentStrums.members[i].visible = false;
			...
			startTimer = new FlxTimer().start(Conductor.crochet / 1000 / playbackRate, function(tmr:FlxTimer) {
				...
				if ((stageUI == "pixel" || uiPostfix == "-pixel") && CoolUtil.exists(Paths.getPath(path("Date") + '.${Paths.SOUND_EXT}', SOUND)))
					goSound = "Date";
				var num:Array<String> = ["3", "2", "1", goSound];

				var tick:Countdown = THREE;
				var ticks:Array<Countdown> = [THREE, TWO, ONE, GO, START];
				if (swagCounter >= 0 && swagCounter <= 4) {
					tick = ticks[swagCounter];

					if (swagCounter < 4) {
						FlxG.sound.play(Paths.sound(path(num[swagCounter])), 0.6);

						var spr = swagCounter > 0 ? createCountdownSprite(introAlts[swagCounter - 1], antialias) : null;
						if (swagCounter == 1) countdownReady = spr;
						else if (swagCounter == 2) countdownSet = spr;
						else if (swagCounter == 3) countdownGo = spr;
					}
				}
	...
	function startSong():Void {
		...
		#if DISCORD_ALLOWED // Updating Discord Rich Presence (with Time Left)
	...
	private function generateSong():Void {
		...
		try {
			var eventsChart = Song.getChart("events", songName);
			if (eventsChart != null)
		...
		for (section in sectionsData) {
			...
			for (i in 0...section.sectionNotes.length) {
				...
				var swagNote = new Note(spawnTime, noteColumn, oldNote);
				...
				if (roundSus > 0) {
					for (susNote in 0...roundSus) {
						...
						var sustainNote = new Note(spawnTime + (curStepCrochet * susNote), noteColumn, oldNote, true);
				...
				if (swagNote.mustPress)
					...
					swagNote.x += 310;
					if (noteColumn > 1) swagNote.x += FlxG.width / 2 + 25; // Up and Right
				}
	...
	function eventPushed(event:EventNote) { // called only once per different event (Used for precaching)
	...
	function eventPushedUnique(event:EventNote) { // called by every event with the same name
		switch (event.event) {
			case "Play Sound": Paths.sound(event.value1); //Precache sound
	...
	override public function update(elapsed:Float) {
		...
		if (camZooming) {
			var int = (-elapsed * 3.125 * camZoomingDecay * playbackRate);
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, Math.exp(int));
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, Math.exp(int));
			camNOTE.zoom = FlxMath.lerp(1, camNOTE.zoom, Math.exp(int));
		}
	...
	public dynamic function updateIconsScale(elapsed:Float) { // Health icon updaters
		var ltEx = (-elapsed * 9 * playbackRate);
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, Math.exp(ltEx));
		iconP1.scale.set(mult, mult);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, Math.exp(ltEx));
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead && gameOverTimer == null) {
			...
			if (ret != LuaUtils.Function_Stop) {
				...
				#if DISCORD_ALLOWED // Game Over doesn't get his its variable because it's only used here
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch (eventName) {
			...
			case "Screen Shake":
				...
				var targetsArray:Array<FlxCamera> = [camGame, camHUD, camNOTE];
			...
			case "Set Property":
				try {
					...
					var name:String = 'ERROR ("Set Property" Event) - ${e.message.substr(0, len)}';
					#if (LUA_ALLOWED || HSCRIPT_ALLOWED) addTextToDebug(name, FlxColor.RED) #else FlxG.log.warn(name) #end;
				}
			case "Play Sound":
		...
		callOnScripts("onEventCalled", [eventName, value1, value2, flValue1, flValue2, strumTime]);
	}
	...
	public function moveCameraSection(?sec:Null<Int>):Void {
		...
		var noteData = SONG.notes[sec];
		if (noteData == null) return;

		var target:String = (gf != null && noteData.gfSection) ? "gf" : (noteData.mustHitSection ? "bf" : "dad");
		moveCameraToSection(target);
	}

	public function moveCameraToSection(target:String) {
		var isGf = target == "gf" || target == "girlfriend";
		var isDad = target == "dad" || target == "opponent";
		var isBf = target == "bf" || target == "boyfriend";

		var char = isGf ? gf : (isDad ? dad : boyfriend);
		if (char == null) return;

		var mid = char.getMidpoint();
		var offset = isGf ? girlfriendCameraOffset : (isDad ? opponentCameraOffset : boyfriendCameraOffset);
		var dir = (isDad || isGf) ? 1 : -1;

		camFollow.setPosition(
			mid.x + (isGf ? 0 : (isDad ? 150 : -100)) + (char.cameraPosition[0] + offset[0]) * dir,
			mid.y + (isGf ? 0 : -100) + char.cameraPosition[1] + offset[1]
		);

		var scriptTarget = isGf ? "gf" : (isDad ? "dad" : "boyfriend");
		callOnScripts("onMoveCamera", [scriptTarget]);
	}
	...
	public function finishSong(?ignoreNoteOffset:Bool = false):Void {
		...
		else finishTimer = new FlxTimer().start(ClientPrefs.data.noteOffset / 1000, (_) -> endCallback());
	}
	...
	public function endSong() {
		...
		deathCounter = 0;
		seenCutscene = false;
		checkForAchievement(["hype", "two_keys", "ur_good", "ur_bad", "toastie"]);
	...
	function cachePopUpScore() {
		var path = (id:String) -> Paths.image('ui/$id' + uiPostfix);
		for (judges in ratingsData) path('judgements/${uiPrefix}${judges.image}');
		for (i in 0...10) path('combo/${uiPrefix}num$i');
	}
	...
	public var comboGroup:FlxSpriteGroup; // Stores Ratings and Combo Sprites in a group
	public var uiGroup:FlxSpriteGroup; // Stores HUD Objects in a Group
	public var noteGroup:FlxTypedGroup<FlxBasic>; // Stores Note Objects in a Group
	...
	function popUpScore(note:Note = null):Void {
		...
		var judges = Conductor.judgeNote(ratingsData, noteDiff / playbackRate);
		totalNotesHit += judges.ratingMod;
		note.ratingMod = judges.ratingMod;

		if (!note.ratingDisabled) judges.hits++;
		note.rating = judges.name;
		score = judges.score;

		if (judges.noteSplash && !note.noteSplashData.disabled)
		...
		if (stageUI != "normal") antialias = !isPixelStage;

		rating.loadGraphic(Paths.image('ui/judgements/$uiPrefix' + '${judges.image}${uiPostfix}'));
		rating.screenCenter();
		...
		var path = (id:String) -> Paths.image('ui/combo/${id}${uiPostfix}');
		var comboSpr = new FlxSprite().loadGraphic(path('${uiPrefix}combo'));
		...
		for (i in 0...separatedScore.length) {
			var numScore = new FlxSprite().loadGraphic(path('${uiPrefix}num' + Std.parseInt(separatedScore.charAt(i))));
	...
	function onKeyPress(event:KeyboardEvent):Void {
		...
		if (!controls.controllerMode) {
			#if debug //Prevents crash specifically on debug without needing to try catch shit
	...
	function keyPressed(key:Int) {
		...
		var lastTime:Float = Conductor.songPosition; //more accurate hit time for the ratings? part1
		...
		if (plrInputNotes.length != 0) { // slightly faster than doing `> 0`
			var funnyNote:Note = plrInputNotes[0]; // front note
			if (plrInputNotes.length > 1) {
				var doubleNote:Note = plrInputNotes[1];
				if (doubleNote.noteData == funnyNote.noteData) { // if the note has a 0ms distance (is on top of the current note), kill it
	...
	function keysCheck():Void { // Hold notes
	...
	function noteMissPress(direction:Int = 1):Void { //You pressed a key when there was no notes to press for this key
		...
		FlxG.sound.play(Paths.soundRandom("missnote/miss", 1, 3), FlxG.random.float(0.1, 0.2));
	...
	function noteMissCommon(direction:Int, note:Note = null) {
		...
		if (note != null && guitarHeroSustains && note.parent == null) { // GUITAR HERO SUSTAIN CHECK.
	...
	function opponentNoteHit(note:Note):Void {
		...
		var result:Dynamic = callOnLuas("opponentNoteHitPre", [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote, canZoom]);
		if (result != LuaUtils.Function_Stop && result != LuaUtils.Function_StopHScript && result != LuaUtils.Function_StopAll) 
			result = callOnHScript("opponentNoteHitPre", [note, canZoom]);

		if (result == LuaUtils.Function_Stop) return;
		camZooming = (result is Bool) ? result : true;
	...
	public function RecalculateRating(badHit:Bool = false, scoreBop:Bool = true) {
		...
		if (ret != LuaUtils.Function_Stop) {
			...
			if (totalPlayed != 0) { //Prevent divide by 0
				...
				ratingName = ratingStuff[ratingStuff.length - 1][0]; // Rating Name && Uses last string
				if (ratingPercent < 1)
	...
	function checkForAchievement(achievesToCheck:Array<String> = null) {
		...
		if (chartingMode || cpuControlled || achievesToCheck == null) return;
	...
	public function initLuaShader(name:String, ?glslVersion:Int = 120) {
		...
		var name:String = 'Missing shader $name .frag AND .vert files!';
		#if (LUA_ALLOWED || HSCRIPT_ALLOWED) addTextToDebug(name, FlxColor.RED) #else FlxG.log.warn(name) #end;
		#else
