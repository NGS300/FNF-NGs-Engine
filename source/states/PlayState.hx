...
class PlayState extends MusicBeatState {
	...
	override public function create() { //trace('Playback Rate: ' + playbackRate);
		...
		if (curStage == "default") {
			...
			var stageFront = new BGSprite("editors/bgfront", -650, 600, 0.9, 0.9);
	...
	inline private function createCountdownSprite(image:String, antialias:Bool):FlxSprite {
		...
		FlxTween.tween(spr, {/*y: spr.y + 100,*/ alpha: 0}, Conductor.crochet / 1000, {
			...
			onComplete: (_) -> { // function(twn:FlxTween) {
				remove(spr);
				spr.destroy();
			}
		});
	...
	public function doScoreBop():Void {
		...
		scoreTxtTween = FlxTween.tween(scoreTxt.scale, {x: 1, y: 1}, 0.2, {
			onComplete: (_) -> scoreTxtTween = null //function(twn:FlxTween) { scoreTxtTween = null; }
		});
	}
	...
	function startSong():Void {
		...
		if (paused) { //trace('Oopsie doopsie! Paused sound');
			FlxG.sound.music.pause();
			vocals.pause();
			opponentVocals.pause();
		}
	...
	override public function update(elapsed:Float) {
		...
		if (camZooming) {
			...
			camNOTE.zoom = FlxMath.lerp(1, camNOTE.zoom, Math.exp(-elapsed * 3.125 * camZoomingDecay * playbackRate));
		}
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch (eventName) {
			...
			case 'Play Animation': //trace('Anim to play: ' + value1);
		...
		for (name in achievesToCheck) {
			...
			if (name != WeekData.getWeekFileName() + '_nomiss') { // common achievements
				switch (name) {
					...
					#if BASE_GAME_FILES
					case 'debugger': unlock = (songName == 'test' && !usedPractice);
					#end
				}
