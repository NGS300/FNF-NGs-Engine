...
class AchievementsMenuState extends MusicBeatState {
	...
	override function create() {
		...
		for (achievement => data in Achievements.achievements) { // prepare achievement list
		...
		for (option in options) {
			...
			var spr = new FlxSprite(0, Math.floor(grpOptions.members.length / MAX_PER_ROW) * 180).loadGraphic(graphic);
		...
		var box = new FlxSprite(0, -30).makeGraphic(1, 1, FlxColor.BLACK);
		...
		var box = new FlxSprite(0, 570).makeGraphic(1, 1, FlxColor.BLACK);
...
class ResetAchievementSubstate extends MusicBeatSubstate {
	...
	public function new() {
		...
		var text = new Alphabet(0, 180, Language.getPhrase("reset_achievement", "Reset Achievement:"), true);
		text.screenCenter(X);
		text.scrollFactor.set();
		add(text);
		
		var state:AchievementsMenuState = cast FlxG.state;
		var text = new FlxText(50, text.y + 90, FlxG.width - 100, state.options[state.curSelected].displayName, 40);
