...
class CreditsState extends MusicBeatState {
	...
	override function create() {
		#if DISCORD_ALLOWED DiscordClient.changePresence("In the Menus", null); #end // Updating Discord Rich Presence
		...
		for (i => credit in creditsStuff) {
			...
			if (isSelectable) {
				...
				var path:String = "icons/credits";
				var str:String = '$path/missing_icon';
				if (credit[1] != null && credit[1].length > 0) {
					var fileName = '$path/' + credit[1];
	...
	function changeSelection(change:Int = 0) {
		...
		var newColor = CoolUtil.colorFromString(creditsStuff[curSelected][4]);
