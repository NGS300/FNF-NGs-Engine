...
class ResetScoreSubState extends MusicBeatSubstate {
	...
	public function new(song:String, difficulty:Int, character:String, week:Int = -1) { // Week -1 = Freeplay
		this.song = song;
		...
		var text = new Alphabet(0, 180, Language.getPhrase("reset_score", "Reset the score of"), true);
		text.screenCenter(X);
		alphabetArray.push(text);
		text.alpha = 0;
		add(text);

		var text = new Alphabet(0, text.y + 90, name, true);
		text.scaleX = tooLong;
		text.screenCenter(X);
